...
const handleLogin = async () => {
   const ok = await userStore.handleLogin( email.value,password.value);
   
   if(ok){
    // Kiểm tra role từ userInfor để redirect đúng trang
    const role = userStore.userInfor?.role
...
<template>
    <div class=" flex  items-center justify-around">
        ...
        <form @submit.prevent="handleLogin" class=" shadow-md w-[500px] rounded-md overflow-hidden bg-[#f2e6eb]" >
                <div class="py-0 px-8">
                    <div class="flex justify-between items-center pt-[10px] py-3 pb-0 ">
                    <h3 class="text-[1.6rem] font-medium text-[var(--text-color)]">Đăng nhập</h3>
                    <router-link :to="{name: 'RegisterPage'}" class="text-[1.2rem] text-[var(--primary-color)] cursor-pointer" >Đăng ký</router-link>
                    </div>
                <div class="mt-5">
                ...
                <div class="flex justify-end mt-4">
                    <span class="text-[var(--primary-color)] font-medium cursor-pointer">Quên mật khẩu</span>
                    <span class="border-r-[1px] border-[#a0a0a0] h-4 mx-2 mt-1 "></span>
                    <span href="" class=" cursor-pointer ">Cần trợ giúp ?</span>
                </div>
                <div class="flex justify-end my-10">
                    ...
                    <button type="submit" class="w-[120px] py-1 px-2 bg-[var(--primary-color)] rounded-sm text-white ml-3">ĐĂNG NHẬP </button>                 
                </div>
            </div>
